...
def datetime_is_after_consent(value):
    ...
    now = value
    if value != now:
        raise ValidationError(u'Date and time cannot be prior to consent date. You entered {}'.format(value))
...
def date_not_before_study_start(value_date):
    ...
    if value_datetime < settings.STUDY_OPEN_DATETIME:
        raise ValidationError(
            'Invalid date. Study opened on {}. Got {}.'.format(settings.STUDY_OPEN_DATETIME, value_date))


def datetime_not_before_study_start(value_datetime):
    if value_datetime < settings.STUDY_OPEN_DATETIME:
        raise ValidationError(
            'Invalid date. Study opened on {}. Got {}.'.format(settings.STUDY_OPEN_DATETIME, value_datetime))
